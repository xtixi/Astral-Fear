...
namespace MD_Plugin
{
    ...
    public class MDM_SculptingPro : MonoBehaviour
    {
        ...
        //----------------------------------------------------------------------------
        //-Big thanks to the community for optimizing suggestions!--------------------
        //----------------------------------------------------------------------------
        ...
        public bool SS_AutoRecalculateNormals = false;
        ...
        [Range(2,20)] public int SS_MultithreadingProcessDelay = 10;
        ...
        public List<Vector3> WorkingVertices = new List<Vector3>();
        ...
        public int[] ThreadTriangles;

        public bool SSInternal_Set = false;
        public bool SSCreateNewReference = true; 
        //If sculpting component is added to the object, the new mesh reference will be created 
        //(this may be changed in case of keeping the original mesh data, but it's highly recommended to keep it true)

        private void Awake()
        {
            if (SSInternal_Set) return;

            if (SSCreateNewReference) MD_MeshProEditor.MeshEditor_STATIC_CreateNewReference(this.gameObject);
        ...
        private void Start()
        {
            if (mesh == null || (mesh && mesh.sharedMesh == null)) return;
            ...
            StoredVertices = new List<Vector3>();
            StoredVertices.AddRange(mesh.sharedMesh.vertices);

            WorkingVertices = new List<Vector3>();
            WorkingVertices.AddRange(mesh.sharedMesh.vertices);

            if (SS_MultithreadingSupported)
            {
                ThreadVertices = mesh.sharedMesh.vertices;

                if(SS_UseSmoothFunct && Application.isPlaying)
                    ThreadTriangles = mesh.sharedMesh.triangles;
        ...
        public void SS_Funct_RestoreOriginal()
        {
            ...
            if (StoredVertices.Count == 0) return;

            mesh.sharedMesh.SetVertices(StoredVertices);
            if (SS_MultithreadingSupported) ThreadVertices = mesh.sharedMesh.vertices;
            SS_Funct_RefreshMeshCollider();
        }
        ...
        {
            if(SS_AutoRecalculateNormals) mesh.sharedMesh.RecalculateNormals();
            ...
            if (SS_MultithreadingSupported)
            {
                ...
                mesh.sharedMesh.vertices = ThreadVertices;
            ...
            while (i < WorkingVertices.Count)
            {
                if (Vector3.Distance(transform.TransformPoint(WorkingVertices[i]), WorldPoint) < Radius)
                {
                    ...
                    if (State == SS_State_Internal.Revert) WorkingVertices[i] = Vector3.Lerp(WorkingVertices[i], StoredVertices[i], 0.1f);
                    else
                    {
            ...
            mesh.sharedMesh.SetVertices(WorkingVertices);
        }
        ...
        { 
            while (true)
            {
                ...
                if (Multithreading_Manager.mth_State == SS_State_Internal.Smooth)
                {
                    ...
                    {
                        case 0:
                            ThreadVertices = MD_MeshUtilities.smoothing_HCFilter.HCFilter(ThreadVertices, ThreadTriangles.ToArray(), new MD_MeshUtilities.SculptingAttributes()
                            {
                            ...
                            break;
                        case 1:
                            ThreadVertices = MD_MeshUtilities.smoothing_LaplacianFilter.LaplacianFilter(ThreadVertices, ThreadTriangles.ToArray(), Multithreading_Manager.mth_SmoothIntensity, new MD_MeshUtilities.SculptingAttributes()
                            {
                                radius = Multithreading_Manager.mth_Radius,
                                transPos = Multithreading_Manager.mth_WorldPos,
                                transRot = Multithreading_Manager.mth_WorldRot,
                    ...
                    while (i < ThreadVertices.Length)
                    {
                        if (Vector3.Distance(ThreadVertices[i], Multithreading_Manager.mth_WorldPoint) < Multithreading_Manager.mth_Radius)
                        {
                            if (Multithreading_Manager.mth_State == SS_State_Internal.Stylize)
                            {
                                ...
                                Vector3 ttt = MD_MeshUtilities.CustomLerp(ThreadVertices[i], v, Multithreading_Manager.mth_StylizeIntensity);
                            ...
                            if (Multithreading_Manager.mth_State == SS_State_Internal.Revert)
                                ThreadVertices[i] = Vector3.Lerp(ThreadVertices[i], StoredVertices[i], 0.1f);

                            Vector3 origin = MD_MeshUtilities.TransformPoint(Multithreading_Manager.mth_WorldPos, Multithreading_Manager.mth_WorldRot, Multithreading_Manager.mth_WorldScale, ThreadVertices[i]);
                            ...
                            if (Multithreading_Manager.mth_State != SS_State_Internal.Revert)
                            {
                                ...
                                else if (SS_MeshSculptMode == SS_MeshSculptMode_Internal.CustomDirectionObject)
                                {
                                    ...
                                    Dir = MD_MeshUtilities.TransformDirection(Multithreading_Manager.mth_WorldPos, Multithreading_Manager.mth_WorldRot, Multithreading_Manager.mth_WorldScale, -ThreadVertices[i]);
                                ...
                                ThreadVertices[i] = MD_MeshUtilities.TransformPointInverse(Multithreading_Manager.mth_WorldPos, Multithreading_Manager.mth_WorldRot, Multithreading_Manager.mth_WorldScale, origin);
                            }
                        }
                        i++;
                    }
