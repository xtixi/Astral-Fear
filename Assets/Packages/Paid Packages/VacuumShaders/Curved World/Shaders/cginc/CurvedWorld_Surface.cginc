...
struct Input 
{
	...
	#ifndef V_CW_TESSELATION
		float4 color : COLOR0;
	#endif

	#if defined(V_CW_RIM) || defined(V_CW_REFLECTIVE)
		float3 viewDir;
	#endif
	...
	#if defined(_NORMALMAP) || defined(V_CW_REFLECTIVE)
		INTERNAL_DATA
	#endif
...
#endif
{
	...
	#ifndef V_CW_TESSELATION
		o.Albedo *= lerp(float3(1, 1, 1), IN.color.rgb, _V_CW_IncludeVertexColor);
	#endif
	...
	//Curved World Distance Fade
		...
		CURVEDWORLD_RANGE_FADE_CALCULATE(IN)
		...
		#endif
		#ifdef V_CW_TRANSPARENT
			float rangeFade = 1 - CURVEDWORLD_RANGE_FADE_VALUE;

			o.Albedo *= rangeFade;
			o.Alpha  *= rangeFade;
			o.Emission *= rangeFade;
			...
			#ifdef V_CW_STANDARD
				o.Metallic *= rangeFade;
				o.Smoothness *= rangeFade;
			#endif
