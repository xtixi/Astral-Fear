...
Shader "Dirt_floor_HMTB"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			#endif
			CBUFFER_END
			TEXTURE2D(_TopDisplacement);
			SAMPLER(sampler_TopDisplacement);
			TEXTURE2D(_BaseDisplacement);
			SAMPLER(sampler_BaseDisplacement);
			...
			TEXTURE2D(_TopSmoothnessAO);
			SAMPLER(sampler_TopSmoothnessAO);
			TEXTURE2D(_BaseSmoothnessMix);
			SAMPLER(sampler_BaseSmoothnessMix);
			TEXTURE2D(_SmoothnessAOBase);
			SAMPLER(sampler_SmoothnessAOBase);
			...
			PackedVaryingsMeshToPS VertexFunction(AttributesMesh inputMesh )
			{
				...
				float4 lerpResult141 = lerp( ( ( SAMPLE_TEXTURE2D_LOD( _TopDisplacement, sampler_TopDisplacement, UVManipulation97, 0.0 ) - temp_cast_0 ) * float4( inputMesh.normalOS , 0.0 ) * _DirtDisplacement ) , ( ( SAMPLE_TEXTURE2D_LOD( _BaseDisplacement, sampler_BaseDisplacement, uv_BaseDisplacement, 0.0 ) - temp_cast_2 ) * float4( inputMesh.normalOS , 0.0 ) * _ConcreteDisplacement ) , DirtMask94);
						...
						)
			{
				...
				float HeightMask163 = saturate(pow(((abs( ( simpleNoise218 * simpleNoise146 ) )*abs( saturate( (0.0 + (abs( length( texCoord41 ) ) - _MaskGain) * (1.0 - 0.0) / (_MaskConstrast - _MaskGain)) ) ))*4)+(abs( saturate( (0.0 + (abs( length( texCoord41 ) ) - _MaskGain) * (1.0 - 0.0) / (_MaskConstrast - _MaskGain)) ) )*2),_BlendStrength));
				float DirtMask94 = saturate( HeightMask163 );
				float4 lerpResult93 = lerp( ( _DirtTint * SAMPLE_TEXTURE2D( _TopAlbedo, sampler_TopAlbedo, UVManipulation97 ) ) , ( _ConcreteTint * SAMPLE_TEXTURE2D( _BaseAlbedo, sampler_BaseAlbedo, uv_BaseAlbedo ) ) , DirtMask94);
				
				float3 unpack223 = UnpackNormalScale( SAMPLE_TEXTURE2D( _TopNormal, sampler_TopNormal, UVManipulation97 ), _DirtNormalIntensity );
				...
				float3 unpack92 = UnpackNormalScale( SAMPLE_TEXTURE2D( _BaseNormal, sampler_BaseNormal, uv_BaseNormal ), _ConcreteNormalIntensity );
				...
				float4 tex2DNode221 = SAMPLE_TEXTURE2D( _TopSmoothnessAO, sampler_TopSmoothnessAO, UVManipulation97 );
				float2 uv_BaseSmoothnessMix = packedInput.ase_texcoord5.xy * _BaseSmoothnessMix_ST.xy + _BaseSmoothnessMix_ST.zw;
				float lerpResult107 = lerp( ( tex2DNode221.r * _DirtSmoothnessIntensity ) , ( SAMPLE_TEXTURE2D( _BaseSmoothnessMix, sampler_BaseSmoothnessMix, uv_BaseSmoothnessMix ).r * _ConcreteSmoothnessIntensity ) , DirtMask94);
				
				float2 uv_SmoothnessAOBase = packedInput.ase_texcoord5.xy * _SmoothnessAOBase_ST.xy + _SmoothnessAOBase_ST.zw;
				float lerpResult109 = lerp( tex2DNode221.g , SAMPLE_TEXTURE2D( _SmoothnessAOBase, sampler_SmoothnessAOBase, uv_SmoothnessAOBase ).g , DirtMask94);
		...
		Pass
		{
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				BuiltinData builtinData;
				GlobalSurfaceDescription surfaceDescription = (GlobalSurfaceDescription)0;
				float2 texCoord16 = packedInput.ase_texcoord.xy * _Tiling + _Offset;
				float cos17 = cos( radians( _Rotation ) );
				float sin17 = sin( radians( _Rotation ) );
				float2 rotator17 = mul( texCoord16 - float2( 0.5,0.5 ) , float2x2( cos17 , -sin17 , sin17 , cos17 )) + float2( 0.5,0.5 );
				float2 UVManipulation97 = rotator17;
				float2 uv_BaseAlbedo = packedInput.ase_texcoord.xy * _BaseAlbedo_ST.xy + _BaseAlbedo_ST.zw;
				float2 texCoord216 = packedInput.ase_texcoord.xy * float2( 1,1 ) + float2( 0,0 );
				float simpleNoise218 = SimpleNoise( texCoord216*_MaskShapeScale );
				float4 appendResult204 = (float4(_NoiseTileX , _NoiseTileY , 0.0 , 0.0));
				float4 appendResult211 = (float4(_NoiseoffsetX , _NoiseoffsetY , 0.0 , 0.0));
				float2 texCoord147 = packedInput.ase_texcoord.xy * appendResult204.xy + appendResult211.xy;
				float simpleNoise146 = SimpleNoise( texCoord147*_MaskDetailScale );
				float4 appendResult210 = (float4(_MaskTilingX , _MaskTilingY , 0.0 , 0.0));
				float4 appendResult207 = (float4(_MaskPositionX , _MaskPositionY , 0.0 , 0.0));
				float2 texCoord41 = packedInput.ase_texcoord.xy * appendResult210.xy + appendResult207.xy;
				float HeightMask163 = saturate(pow(((abs( ( simpleNoise218 * simpleNoise146 ) )*abs( saturate( (0.0 + (abs( length( texCoord41 ) ) - _MaskGain) * (1.0 - 0.0) / (_MaskConstrast - _MaskGain)) ) ))*4)+(abs( saturate( (0.0 + (abs( length( texCoord41 ) ) - _MaskGain) * (1.0 - 0.0) / (_MaskConstrast - _MaskGain)) ) )*2),_BlendStrength));
				float DirtMask94 = saturate( HeightMask163 );
				float4 lerpResult93 = lerp( ( _DirtTint * SAMPLE_TEXTURE2D( _TopAlbedo, sampler_TopAlbedo, UVManipulation97 ) ) , ( _ConcreteTint * SAMPLE_TEXTURE2D( _BaseAlbedo, sampler_BaseAlbedo, uv_BaseAlbedo ) ) , DirtMask94);
				...
				float3 unpack223 = UnpackNormalScale( SAMPLE_TEXTURE2D( _TopNormal, sampler_TopNormal, UVManipulation97 ), _DirtNormalIntensity );
				unpack223.z = lerp( 1, unpack223.z, saturate(_DirtNormalIntensity) );
				float2 uv_BaseNormal = packedInput.ase_texcoord.xy * _BaseNormal_ST.xy + _BaseNormal_ST.zw;
				float3 unpack92 = UnpackNormalScale( SAMPLE_TEXTURE2D( _BaseNormal, sampler_BaseNormal, uv_BaseNormal ), _ConcreteNormalIntensity );
				unpack92.z = lerp( 1, unpack92.z, saturate(_ConcreteNormalIntensity) );
				float3 lerpResult103 = lerp( unpack223 , unpack92 , DirtMask94);
				float3 normalizeResult105 = normalize( lerpResult103 );
				...
				float4 tex2DNode221 = SAMPLE_TEXTURE2D( _TopSmoothnessAO, sampler_TopSmoothnessAO, UVManipulation97 );
				float2 uv_BaseSmoothnessMix = packedInput.ase_texcoord.xy * _BaseSmoothnessMix_ST.xy + _BaseSmoothnessMix_ST.zw;
				float lerpResult107 = lerp( ( tex2DNode221.r * _DirtSmoothnessIntensity ) , ( SAMPLE_TEXTURE2D( _BaseSmoothnessMix, sampler_BaseSmoothnessMix, uv_BaseSmoothnessMix ).r * _ConcreteSmoothnessIntensity ) , DirtMask94);
				
				float2 uv_SmoothnessAOBase = packedInput.ase_texcoord.xy * _SmoothnessAOBase_ST.xy + _SmoothnessAOBase_ST.zw;
				float lerpResult109 = lerp( tex2DNode221.g , SAMPLE_TEXTURE2D( _SmoothnessAOBase, sampler_SmoothnessAOBase, uv_SmoothnessAOBase ).g , DirtMask94);
...
/*ASEBEGIN
Version=18920
303;172;1497;869;6088.829;3010.97;4.930737;True;False
...
WireConnection;0;8;109;0
WireConnection;0;11;141;0
ASEEND*/
//CHKSM=DC6BDA33A348E7346B183FE0499712646C92F925