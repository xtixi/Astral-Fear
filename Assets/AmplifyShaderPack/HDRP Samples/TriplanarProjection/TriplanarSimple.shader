...
Shader "AmplifyShaderPack/HDRP/Triplanar Simple"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			SAMPLER(sampler_TriplanarNormal);
			...
			inline float4 TriplanarSampling343( TEXTURE2D(topTexMap), SAMPLER(samplertopTexMap), TEXTURE2D(midTexMap), SAMPLER(samplermidTexMap), TEXTURE2D(botTexMap), SAMPLER(samplerbotTexMap), float3 worldPos, float3 worldNormal, float falloff, float2 tiling, float3 normalScale, float3 index )
			{
			...
			inline float3 TriplanarSampling345( TEXTURE2D(topTexMap), SAMPLER(samplertopTexMap), TEXTURE2D(midTexMap), SAMPLER(samplermidTexMap), TEXTURE2D(botTexMap), SAMPLER(samplerbotTexMap), float3 worldPos, float3 worldNormal, float falloff, float2 tiling, float3 normalScale, float3 index )
			{
				float3 projNormal = ( pow( abs( worldNormal ), falloff ) );
				projNormal /= ( projNormal.x + projNormal.y + projNormal.z ) + 0.00001;
				float3 nsign = sign( worldNormal );
						...
						)
			{
				...
				float4 triplanar343 = TriplanarSampling343( _TopAlbedo, sampler_TopAlbedo, _TriplanarAlbedo, sampler_TriplanarAlbedo, _TriplanarAlbedo, sampler_TriplanarAlbedo, ase_worldPos, normalWS, _CoverageFalloff, float2( 1,1 ), float3( 1,1,1 ), float3(0,0,0) );
				...
				float3 triplanar345 = TriplanarSampling345( _TopNormal, sampler_TopNormal, _TriplanarNormal, sampler_TriplanarNormal, _TriplanarNormal, sampler_TriplanarNormal, ase_worldPos, normalWS, _CoverageFalloff, float2( 1,1 ), float3( 1,1,1 ), float3(0,0,0) );
				float3 tanTriplanarNormal345 = mul( ase_worldToTangent, triplanar345 );
				
				float3 temp_cast_1 = (_Specular).xxx;
				
				surfaceDescription.Albedo = triplanar343.xyz;
		...
		Pass
		{
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				float4 triplanar343 = TriplanarSampling343( _TopAlbedo, sampler_TopAlbedo, _TriplanarAlbedo, sampler_TriplanarAlbedo, _TriplanarAlbedo, sampler_TriplanarAlbedo, ase_worldPos, ase_worldNormal, _CoverageFalloff, float2( 1,1 ), float3( 1,1,1 ), float3(0,0,0) );
				...
				float3 triplanar345 = TriplanarSampling345( _TopNormal, sampler_TopNormal, _TriplanarNormal, sampler_TriplanarNormal, _TriplanarNormal, sampler_TriplanarNormal, ase_worldPos, ase_worldNormal, _CoverageFalloff, float2( 1,1 ), float3( 1,1,1 ), float3(0,0,0) );
				float3 tanTriplanarNormal345 = mul( ase_worldToTangent, triplanar345 );
...
/*ASEBEGIN
Version=18920
303;172;1497;869;-932.946;40.70255;1;True;False
...
Node;AmplifyShaderEditor.RangedFloatNode;213;1184,624;Float;False;Property;_Smoothness;Smoothness;5;0;Create;True;0;0;0;False;0;False;0.5;0.25;0;1;0;1;FLOAT;0
...
ASEEND*/
//CHKSM=EBC483D1F8FC4843DE681C1EFC77472BBEC36568