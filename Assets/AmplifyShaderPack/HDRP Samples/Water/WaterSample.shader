...
Shader "AmplifyShaderPack/HDRP/WaterSample"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
						...
						)
			{
				...
				float4 fetchOpaqueVal65 = ASEHDSampleSceneColor(( float3( (ase_grabScreenPosNorm).xy ,  0.0 ) + ( temp_output_24_0 * _Distortion ) ).xy, 0.0, GetInverseCurrentExposureMultiplier());
		...
		Pass
		{
						...
						)
			{
				...
				float temp_output_94_0 = saturate( pow( ( temp_output_89_0 + _WaterDepth ) , _WaterFalloff ) );
				float4 lerpResult13 = lerp( _DeepColor , _ShalowColor , temp_output_94_0);
				float2 uv_Foam = packedInput.ase_texcoord8.xy * _Foam_ST.xy + _Foam_ST.zw;
				float2 panner116 = ( 1.0 * _Time.y * float2( -0.01,0.01 ) + uv_Foam);
				float temp_output_114_0 = ( saturate( pow( ( temp_output_89_0 + _FoamDepth ) , _FoamFalloff ) ) * tex2D( _Foam, panner116 ).r );
				float4 lerpResult117 = lerp( lerpResult13 , float4(1,1,1,0) , temp_output_114_0);
				float4 ase_grabScreenPos = ASE_ComputeGrabScreenPos( screenPos );
				float4 ase_grabScreenPosNorm = ase_grabScreenPos / ase_grabScreenPos.w;
				float2 uv_WaterNormal = packedInput.ase_texcoord8.xy * _WaterNormal_ST.xy + _WaterNormal_ST.zw;
				float2 panner22 = ( 1.0 * _Time.y * float2( -0.03,0 ) + uv_WaterNormal);
				float3 unpack23 = UnpackNormalScale( tex2D( _WaterNormal, panner22 ), _NormalScale );
				unpack23.z = lerp( 1, unpack23.z, saturate(_NormalScale) );
				float2 panner19 = ( 1.0 * _Time.y * float2( 0.04,0.04 ) + uv_WaterNormal);
...
/*ASEBEGIN
Version=18920
303;172;1497;869;-472.4059;999.9864;1.337952;True;False
...
Node;AmplifyShaderEditor.ColorNode;12;-697.5002,-417.5007;Float;False;Property;_DeepColor;Deep Color;2;0;Create;True;0;0;0;False;0;False;0,0,0,0;0,0.04310156,0.249998,0;False;0;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
...
WireConnection;173;5;130;0
WireConnection;173;7;133;0
ASEEND*/
//CHKSM=ED7F0451C30B52CD15C975AA14E375302454942C