...
Shader "AmplifyShaderPack/HDRP/SnowAccum"
{
	Properties
	{
		...
		[ASEEnd]_SnowSpecMultiplier("Snow Spec Multiplier", Float) = 0
	...
	SubShader
	{
		...
		Pass
		{
			...
			TEXTURE2D(_RockSpecular);
			SAMPLER(sampler_RockSpecular);
			TEXTURE2D(_SnowSpecular);
			SAMPLER(sampler_SnowSpecular);
			...
			PackedVaryingsMeshToPS VertexFunction(AttributesMesh inputMesh )
			{
				...
				float2 uv_RockNormal = inputMesh.ase_texcoord.xy * _RockNormal_ST.xy + _RockNormal_ST.zw;
				float2 uv_SnowNormal = inputMesh.ase_texcoord.xy * _SnowNormal_ST.xy + _SnowNormal_ST.zw;
				float3 ase_worldNormal = TransformObjectToWorldNormal(inputMesh.normalOS);
				float3 lerpResult15 = lerp( UnpackNormalScale( SAMPLE_TEXTURE2D_LOD( _RockNormal, sampler_RockNormal, uv_RockNormal, 1.0 ), 1.0f ) , UnpackNormalScale( SAMPLE_TEXTURE2D_LOD( _SnowNormal, sampler_SnowNormal, uv_SnowNormal, 1.0 ), 1.0f ) , saturate( ( ase_worldNormal.y * _SnowAmount ) ));
				...
				#endif
				float3 vertexValue = ( temp_output_18_0 * inputMesh.normalOS * (0.0 + (temp_output_18_0 - 0.0) * (0.04 - 0.0) / (1.0 - 0.0)) );
						...
						)
			{
				...
				float3 lerpResult15 = lerp( UnpackNormalScale( SAMPLE_TEXTURE2D( _RockNormal, sampler_RockNormal, uv_RockNormal ), 1.0f ) , UnpackNormalScale( SAMPLE_TEXTURE2D( _SnowNormal, sampler_SnowNormal, uv_SnowNormal ), 1.0f ) , saturate( ( normalWS.y * _SnowAmount ) ));
				...
				float3 worldNormal19 = float3(dot(tanToWorld0,tanNormal19), dot(tanToWorld1,tanNormal19), dot(tanToWorld2,tanNormal19));
				float temp_output_18_0 = saturate( ( worldNormal19.y * _SnowAmount ) );
				float4 lerpResult10 = lerp( SAMPLE_TEXTURE2D( _RockAlbedo, sampler_RockAlbedo, uv_RockAlbedo ) , SAMPLE_TEXTURE2D( _SnowAlbedo, sampler_SnowAlbedo, uv_SnowAlbedo ) , temp_output_18_0);
				...
				float4 lerpResult17 = lerp( SAMPLE_TEXTURE2D( _RockSpecular, sampler_RockSpecular, uv_RockSpecular ) , ( SAMPLE_TEXTURE2D( _SnowSpecular, sampler_SnowSpecular, uv_SnowSpecular ) * _SnowSpecMultiplier ) , temp_output_18_0);
		...
		Pass
		{
			...
			PackedVaryingsMeshToPS VertexFunction(AttributesMesh inputMesh  )
			{
				...
				float2 uv_RockNormal = inputMesh.uv0.xy * _RockNormal_ST.xy + _RockNormal_ST.zw;
				float2 uv_SnowNormal = inputMesh.uv0.xy * _SnowNormal_ST.xy + _SnowNormal_ST.zw;
				float3 ase_worldNormal = TransformObjectToWorldNormal(inputMesh.normalOS);
				float3 lerpResult15 = lerp( UnpackNormalScale( SAMPLE_TEXTURE2D_LOD( _RockNormal, sampler_RockNormal, uv_RockNormal, 1.0 ), 1.0f ) , UnpackNormalScale( SAMPLE_TEXTURE2D_LOD( _SnowNormal, sampler_SnowNormal, uv_SnowNormal, 1.0 ), 1.0f ) , saturate( ( ase_worldNormal.y * _SnowAmount ) ));
				float3 ase_worldTangent = TransformObjectToWorldDir(inputMesh.tangentOS.xyz);
				float ase_vertexTangentSign = inputMesh.tangentOS.w * unity_WorldTransformParams.w;
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				BuiltinData builtinData;
				GlobalSurfaceDescription surfaceDescription = (GlobalSurfaceDescription)0;
				float2 uv_RockAlbedo = packedInput.ase_texcoord.xy * _RockAlbedo_ST.xy + _RockAlbedo_ST.zw;
				float2 uv_SnowAlbedo = packedInput.ase_texcoord.xy * _SnowAlbedo_ST.xy + _SnowAlbedo_ST.zw;
				float2 uv_RockNormal = packedInput.ase_texcoord.xy * _RockNormal_ST.xy + _RockNormal_ST.zw;
				...
				float3 lerpResult15 = lerp( UnpackNormalScale( SAMPLE_TEXTURE2D( _RockNormal, sampler_RockNormal, uv_RockNormal ), 1.0f ) , UnpackNormalScale( SAMPLE_TEXTURE2D( _SnowNormal, sampler_SnowNormal, uv_SnowNormal ), 1.0f ) , saturate( ( ase_worldNormal.y * _SnowAmount ) ));
				...
				float2 uv_RockSpecular = packedInput.ase_texcoord.xy * _RockSpecular_ST.xy + _RockSpecular_ST.zw;
				float2 uv_SnowSpecular = packedInput.ase_texcoord.xy * _SnowSpecular_ST.xy + _SnowSpecular_ST.zw;
				float4 lerpResult17 = lerp( SAMPLE_TEXTURE2D( _RockSpecular, sampler_RockSpecular, uv_RockSpecular ) , ( SAMPLE_TEXTURE2D( _SnowSpecular, sampler_SnowSpecular, uv_SnowSpecular ) * _SnowSpecMultiplier ) , temp_output_18_0);
...
/*ASEBEGIN
Version=18920
303;172;1497;869;3012.881;948.3238;2.974217;True;False
Node;AmplifyShaderEditor.RangedFloatNode;12;-1773.599,-115.1001;Float;False;Property;_SnowAmount;SnowAmount;6;0;Create;True;0;0;0;False;0;False;0.13;2;0;2;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.RangedFloatNode;34;-857.3526,714.2256;Inherit;False;Property;_SnowSpecMultiplier;Snow Spec Multiplier;7;0;Create;True;0;0;0;False;0;False;0;7.48;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.SamplerNode;16;-990.9933,493.5407;Inherit;True;Property;_SnowSpecular;Snow Specular;5;0;Create;True;0;0;0;False;0;False;-1;None;a367220094f74e1c8ae143c4b538ba8d;True;0;False;white;Auto;False;Object;-1;Auto;Texture2D;8;0;SAMPLER2D;0,0;False;1;FLOAT2;0,0;False;2;FLOAT;1;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;6;FLOAT;0;False;7;SAMPLERSTATE;;False;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.SaturateNode;18;-496.7049,-110.1974;Inherit;False;1;0;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;35;-455.1855,437.7484;Inherit;False;2;2;0;COLOR;0,0,0,0;False;1;FLOAT;0;False;1;COLOR;0
...
Node;AmplifyShaderEditor.SamplerNode;2;-710.1995,223.4709;Inherit;True;Property;_RockSpecular;Rock Specular;2;0;Create;True;0;0;0;False;0;False;-1;None;f4ef9b0dca354d7aaf185497d6b77ea5;True;0;False;white;Auto;False;Object;-1;Auto;Texture2D;8;0;SAMPLER2D;0,0;False;1;FLOAT2;0,0;False;2;FLOAT;1;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;6;FLOAT;0;False;7;SAMPLERSTATE;;False;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.NormalVertexDataNode;36;-323.2021,819.6256;Inherit;False;0;5;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.TFHCRemapNode;39;-297.0844,610.1588;Inherit;False;5;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;1;False;3;FLOAT;0;False;4;FLOAT;0.04;False;1;FLOAT;0
Node;AmplifyShaderEditor.LerpOp;17;-223.4019,294.0071;Inherit;False;3;0;COLOR;0,0,0,0;False;1;COLOR;0,0,0,0;False;2;FLOAT;0;False;1;COLOR;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;37;-5.869408,342.0728;Inherit;False;3;3;0;FLOAT;0;False;1;FLOAT3;0,0,0;False;2;FLOAT;0;False;1;FLOAT3;0
...
WireConnection;23;11;37;0
ASEEND*/
//CHKSM=4B29C9EB911C5A0CE3CC9D1EDCC2E038D34BF4CE