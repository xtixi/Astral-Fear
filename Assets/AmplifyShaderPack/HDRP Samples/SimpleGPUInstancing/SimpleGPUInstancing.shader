...
Shader "AmplifyShaderPack/HDRP/SimpleGPUInstancing"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			CBUFFER_END
			sampler2D _Checkers;
			UNITY_INSTANCING_BUFFER_START(AmplifyShaderPackHDRPSimpleGPUInstancing)
				UNITY_DEFINE_INSTANCED_PROP(float4, _Checkers_ST)
				UNITY_DEFINE_INSTANCED_PROP(float4, _Color)
			UNITY_INSTANCING_BUFFER_END(AmplifyShaderPackHDRPSimpleGPUInstancing)
						...
						)
			{
				...
				float4 _Checkers_ST_Instance = UNITY_ACCESS_INSTANCED_PROP(AmplifyShaderPackHDRPSimpleGPUInstancing,_Checkers_ST);
				float2 uv_Checkers = packedInput.ase_texcoord5.xy * _Checkers_ST_Instance.xy + _Checkers_ST_Instance.zw;
				float4 _Color_Instance = UNITY_ACCESS_INSTANCED_PROP(AmplifyShaderPackHDRPSimpleGPUInstancing,_Color);
				
				float3 hdEmission24 = ASEGetEmissionHDRColor(_Color_Instance.rgb,1.0,1.0,GetInverseCurrentExposureMultiplier());
				
				surfaceDescription.Albedo = ( tex2D( _Checkers, uv_Checkers ) * _Color_Instance ).rgb;
				...
				surfaceDescription.Emission = hdEmission24;
		...
		Pass
		{
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				GlobalSurfaceDescription surfaceDescription = (GlobalSurfaceDescription)0;
				float4 _Checkers_ST_Instance = UNITY_ACCESS_INSTANCED_PROP(AmplifyShaderPackHDRPSimpleGPUInstancing,_Checkers_ST);
				float2 uv_Checkers = packedInput.ase_texcoord.xy * _Checkers_ST_Instance.xy + _Checkers_ST_Instance.zw;
				float4 _Color_Instance = UNITY_ACCESS_INSTANCED_PROP(AmplifyShaderPackHDRPSimpleGPUInstancing,_Color);
		...
		Pass
		{
						...
						)
			{
				...
				GlobalSurfaceDescription surfaceDescription = (GlobalSurfaceDescription)0;
				float4 _Checkers_ST_Instance = UNITY_ACCESS_INSTANCED_PROP(AmplifyShaderPackHDRPSimpleGPUInstancing,_Checkers_ST);
				float2 uv_Checkers = packedInput.ase_texcoord7.xy * _Checkers_ST_Instance.xy + _Checkers_ST_Instance.zw;
				float4 _Color_Instance = UNITY_ACCESS_INSTANCED_PROP(AmplifyShaderPackHDRPSimpleGPUInstancing,_Color);
...
/*ASEBEGIN
Version=18920
303;172;1497;869;1113.5;64.41275;1;True;False
...
Node;AmplifyShaderEditor.RangedFloatNode;23;-425.4297,599.8032;Inherit;False;Constant;_EmissionWeight;Emission Weight;3;0;Create;True;0;0;0;False;0;False;1;1;0;0;0;1;FLOAT;0
...
WireConnection;4;6;24;0
ASEEND*/
//CHKSM=C421CF35C090C62D644AE936D02A4769424C416D